...
BEGIN
	...
	SELECT pOne.[name] as product_name, Sells.price, Seller.[name] as seller_name,
		...
		WHERE Sells.productID = pOne.productID AND Sells.sellerID = Seller.sellerID AND pOne.productID IN (SELECT sellsTwo.productID FROM Sells AS sellsTwo, Criteria WHERE Criteria.client_username = 'DoubloonDan' AND (Criteria.price >= sellsTwo.price OR Criteria.price = null))
	) AS subquery, Criteria
	WHERE Criteria.client_username = 'DoubloonDan' AND (Criteria.product_type = subquery.[type] OR Criteria.product_type = null)
END
