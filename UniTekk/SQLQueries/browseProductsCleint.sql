...
BEGIN
	...
	(
		...
		WHERE Sells.productID = pOne.productID AND Sells.sellerID = Seller.sellerID AND pOne.productID IN 
		(
			SELECT sellsTwo.productID 
			FROM Sells AS sellsTwo, Criteria 
			WHERE Criteria.client_username = @username AND (Criteria.price >= sellsTwo.price OR Criteria.price = null)
		)
	) AS subquery, Criteria
	WHERE Criteria.client_username = @username AND (Criteria.product_type = subquery.[type] OR Criteria.product_type = null)
END